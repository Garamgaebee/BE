...
public class ThreadController {
    ...
    public ResponseEntity<CreateThreadRes> createThread(
            @Parameter(
    ...
    public ResponseEntity<CreateThreadRes> createTeamThread(
            @Parameter(
                    description = "multipart/form-data 형식의 이미지 파일 리스트 (하나라도 리스트로 해서 보내주세요)",
                    content = @Content(mediaType = MediaType.MULTIPART_FORM_DATA_VALUE)
            )
    ...
    public ResponseEntity<DeleteThreadRes> deleteThread(@Parameter(description = "삭제 대상 id (UUID String)") @PathVariable("thread-id") String threadIdx) {
    ...
    public ResponseEntity<List<GetThreadListRes>> getThreadList(@Parameter(description = "0이면 최신순, 1이면 인기순")@RequestParam("order-type") Integer orderType) {
    ...
    public ResponseEntity<CreateCommentRes> createTeamThread(
            @Parameter(
                    description = "multipart/form-data 형식의 이미지 파일 리스트 (하나라도 리스트로 해서 보내주세요)",
                    content = @Content(mediaType = MediaType.MULTIPART_FORM_DATA_VALUE)
            )
    ...
    public ResponseEntity<CreateLikeRes> createLike(@Parameter(schema = @Schema(implementation = CreateLikeCommand.class)) @RequestBody CreateLikeCommand req) {
    ...
    @DeleteMapping("/like")
    public ResponseEntity<DeleteLikeRes> deleteLike(@Parameter(schema = @Schema(implementation = DeleteLikeCommand.class)) @RequestBody DeleteLikeCommand req) {
